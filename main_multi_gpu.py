        ...
        if batch_id % debug_steps == 0:
            ...
            if master_logger and dist.get_rank() == 0:
                master_logger.info(
                    f"Epoch[{epoch:03d}/{total_epochs:03d}], " +
                    f"Step[{batch_id:04d}/{total_batch:04d}], " +
                    f"Loss: {master_train_loss_meter.val:.4f}({master_train_loss_meter.avg:.4f}), " +
                    f"Acc: {master_train_acc_meter.val:.4f}({master_train_acc_meter.avg:.4f})")
        ...
        for batch_id, data in enumerate(dataloader):
            ...
            if batch_id % debug_steps == 0:
                ...
                if master_logger and dist.get_rank() == 0:
                    master_logger.info(
                        f"Val Step[{batch_id:04d}/{total_batch:04d}], " +
                        f"Loss: {master_val_loss_meter.val:.4f} ({master_val_loss_meter.avg:.4f}), " +
                        f"Acc@1: {master_val_acc1_meter.val:.4f} ({master_val_acc1_meter.avg:.4f}), " +
                        f"Acc@5: {master_val_acc5_meter.val:.4f} ({master_val_acc5_meter.avg:.4f})")
...
def main_worker(*args):
    ...
    for epoch in range(last_epoch+1, config.TRAIN.NUM_EPOCHS+1):
        ...
        train_loss, train_acc, train_avg_loss, train_avg_acc, train_time = train(
        ...
        if local_rank == 0:
            if avg_acc1 > best_val_acc1:
                ...
                master_logger.info(f'Max accuracy so far: {avg_acc1:.4f} at epoch_{epoch}')
