...
class CosineLRScheduler(LRScheduler):
    ...
    def __init__(self,
        ...
        self.t_mul = t_mul
        super(CosineLRScheduler, self).__init__(learning_rate, last_epoch, verbose)
    ...
    def get_lr(self):
        ...
        if t < self.warmup_epochs:
            ...
            i = t // self.total_epochs
            t_i = self.total_epochs
            t_curr = t - (self.total_epochs * i)

            gamma = self.decay_rate ** i
            lr_min = self.end_lr * gamma

            val = lr_min + 0.5 * (self.start_lr - lr_min) * (1 + math.cos(math.pi * t_curr / t_i))
    ...
    def get_cycle_length(self, cycles=0):
        ...
        cycles = max(1, cycles)
        if self.t_mul == 1.0:
            return self.total_epochs * cycles
        else:
            return int(math.floor(-self.total_epochs * (self.t_mul ** cycles - 1) / (1 - self.t_mul)))
